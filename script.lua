...
-- VENTANA PRINCIPAL
local window = library:AddWindow("Keey", {
    main_color = Color3.fromRGB(126, 31, 194), -- purple
...
-- ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ ConfiguraciÃƒÆ’Ã‚Â³n optimizada
...
local REPS_PER_CYCLE = 200
local REP_DELAY = 0.00
...
-- 2ÃƒÂ¯Ã‚Â¸Ã‚ÂÃƒÂ¢Ã†â€™Ã‚Â£ Switch: Eat Egg (30 Min)
...
local function eatProteinEggNew()
    ...
    if egg then
        ...
        warn("[AutoEgg] No se encontrÃƒÆ’Ã‚Â³ Protein Egg en Backpack.")
    end
end
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¹ SWITCH PARA ACTIVAR/DESACTIVAR EL ANTI AFK
AutoFarm:AddSwitch("Anti AFK", function(state)
    if state then
        ...
        madebybloodofbatus.BackgroundColor3 = Color3.fromRGB(13, 2, 20)
        ...
        uselesslabelone.Text = "Anti Afk BY BSW_GHOST👻"
        ...
        -- ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¤ Anti-AFK Core
        ...
        -- ÃƒÂ¢Ã‚ÂÃ…â€™ DESACTIVAR ANTI AFK
        getgenv().AntiAfkExecuted = false
        getgenv().zamanbaslaticisi = false
...
AutoFarm:AddButton("Equip Swift Samurai", function()
    print("BotÃƒÆ’Ã‚Â³n presionado: equipando 8 Swift Samurai")
    ...
    -- Ahora equipamos mÃƒÆ’Ã‚Â¡ximo 8 "Swift Samurai"
    local equipped = 0
    local maxEquip = 8
    for _, folder in pairs(petsFolder:GetChildren()) do
        if folder:IsA("Folder") then
...
-- ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â® BotÃƒÆ’Ã‚Â³n: Jungle Squat
AutoFarm:AddButton("Jungle Squat", function()
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        print("[Jungle Squat] AcciÃƒÆ’Ã‚Â³n ejecutada.")
    else
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ FOLDER REBIRTH (Ordenado con Anti AFK antes de Anti Lag)
...
task.spawn(function()
    while task.wait(1) do
        ...
        local gainedRebirths = rebirthsStat.Value - sessionRebirths
        gainedLabel.Text = "Gained Rebirths: " .. tostring(gainedRebirths)
    end
end)

-- ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Fast Rebirths
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ Lock Position
...
-- ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¢ Jungle Lift (Nuevo)
...
-- BotÃƒÆ’Ã‚Â³n para desbloquear el Gamepass AutoLift
...
-- FunciÃƒÆ’Ã‚Â³n para crear switches de auto-equip
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ NUEVO FOLDER ROCKS
...
rebirthsFolder:AddTextBox("Rebirth Target", function(text)
    ...
    if newValue and newValue > 0 then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Entrada InvÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lida",
            Text = "Por favor ingresa un nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lido mayor que 0",
            Duration = 0
        })
    end
end)
        ...
        spawn(function()
            while _G.targetRebirthActive and wait(0.1) do
                ...
                if currentRebirths >= targetRebirthValue then
                    ...
                    game:GetService("StarterGui"):SetCore("SendNotification", {
                        Title = "ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡Objetivo Alcanzado!",
...
end, "Renacimiento automÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡tico hasta alcanzar el objetivo")
...
end, "Establece el tamaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±o del personaje a 1 continuamente")
...
end, "Teletransporte continuo al Rey MÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsculo")
...
local timeDropdown = AutoFarm:AddDropdown("Change Time", function(selection)
    ...
    if selection == "MaÃƒÂ±ana" then
        Lighting.ClockTime = 6
        Lighting.Brightness = 2
        Lighting.Ambient = Color3.fromRGB(200, 200, 255)
    elseif selection == "MediodÃƒÂ­a" then
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…  NUEVA PESTAÃƒÆ’Ã¢â‚¬ËœA: CALCULAR STATS
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€œ Lista de RAWs a ejecutar
local urls = {
...
-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ Teleport / Follow System (versiÃƒÆ’Ã‚Â³n auto-follow desde Dropdown)

-- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ Auto Follow (TP detrÃƒÆ’Ã‚Â¡s del jugador en vez de caminar)
local following = false
local followTarget = nil

-- FunciÃƒÆ’Ã‚Â³n auxiliar: TP detrÃƒÆ’Ã‚Â¡s del jugador
function followPlayer(targetPlayer)
    ...
    if myHRP and targetHRP then
        -- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ Calcular posiciÃƒÆ’Ã‚Â³n detrÃƒÆ’Ã‚Â¡s del jugador (3 studs atrÃƒÆ’Ã‚Â¡s)
        local followPos = targetHRP.Position - (targetHRP.CFrame.LookVector * 3)
        -- ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ Teletransportar siempre recto
        myHRP.CFrame = CFrame.new(followPos, targetHRP.Position)
    end
end

-- Dropdown dinÃƒÆ’Ã‚Â¡mico de jugadores
...
-- BotÃƒÆ’Ã‚Â³n para dejar de seguir
...
-- Loop de seguimiento automÃƒÆ’Ã‚Â¡tico
task.spawn(function()
    while true do
    ...
    if state then
        task.spawn(function()
            while godDamageActive do
                ...
                task.wait(0.1) -- delay pequeÃƒÆ’Ã‚Â±o
            end
        end)
    end
end)
...
-- ÃƒÂ¢Ã…Â¡Ã‚Â¡ BotÃƒÆ’Ã‚Â³n que ejecuta todos los scripts remotos
Killer:AddButton("Stick Dead", function()
...
-- Agregar opciones al dropdown dinÃƒÆ’Ã‚Â¡micamente
for _, option in ipairs(timeOptions) do
    timeDropdown:Add(option)
end

local estadisticas = window:AddTab("Stats Players")
        ...
        if SelectPlayerName ~= "" then
            ...
            if player then
                ...
                if player:FindFirstChild("currentMap") then
                    ...
                    Update9.Text = "Current Map: Aucune donnÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬ ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬ ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©e"
                end
            end
        end
    end
...
local function updateItemCount()
    ...
    if backpack then
        for _, item in ipairs(backpack:GetChildren()) do
            ...
            elseif item.Name == "Tropical Shake" or item.Name == "PiÃƒÆ’Ã‚Â±as" then
